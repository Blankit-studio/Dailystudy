...
- 📌 **오답 노트** — 자주 틀린 카드만 모아 어려운 순으로 보여주고, 그 카드들만 집중 복습할 수 있습니다.
- ⏰ **스트릭 리마인더** — 오늘 학습을 안 했고 연속 기록이 살아 있으면 대시보드에서 알려줍니다. 자정이 가까울수록 강조되고, 기록이 끊긴 뒤에는 다시 시작하도록 격려합니다.
- 🔥 **스트릭 & 통계** — 연속 학습일, 최장 스트릭, 13주 학습 히트맵을 제공합니다.
- 🌍 **다국어 지원** — 13개 언어, 출발어/목표어 조합을 자유롭게 선택(설정에서 인기 조합 원클릭).
- 🎚️ **난이도 선택** — 초·중·고급 레벨에 맞춰 카드·문장이 필터링되고, AI도 해당 수준으로 생성합니다.
...
- **프로덕션에서 `CRON_SECRET`이 없으면 이 엔드포인트는 401로 차단됩니다.**
  (환경 변수를 빠뜨렸을 때 콘텐츠가 무방비로 삭제되는 것을 막기 위함)
...
│  ├─ stats.ts                 # 스트릭/히트맵 계산
│  ├─ difficulty.ts            # 오답 노트 난이도 판정 (ease 기반)
│  ├─ data.ts                  # 서버 데이터 조회
